...
async function main() {
  ...
  let results;
  try {
    results = get(key, { query: prompt, k: 3 });
  } catch {
    process.exit(0);
  }
  ...
  for (const r of results) {
    ...
    let shortSummary = (meta.summary ?? 'unknown topic').slice(0, 80).trim();
    if ((meta.summary ?? '').length > 80) shortSummary += '...';

    const timeAgo = formatTimeAgo(meta.ts ?? '');
